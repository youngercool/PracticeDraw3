...
public class Practice13GetTextBoundsView extends View {
...

    {
        ...
        Rect bound = new Rect();
        paint2.getTextBounds(text1,0,text1.toString().length(),bound);
        yOffsets[0] = -(bound.top + bound.bottom) / 2;
        paint2.getTextBounds(text2,0,text2.toString().length(),bound);
        yOffsets[1] = -(bound.top + bound.bottom) / 2;
        paint2.getTextBounds(text3,0,text3.toString().length(),bound);
        yOffsets[2] = -(bound.top + bound.bottom) / 2;
        paint2.getTextBounds(text4,0,text4.toString().length(),bound);
        yOffsets[3] = -(bound.top + bound.bottom) / 2;
        paint2.getTextBounds(text5,0,text5.toString().length(),bound);
        yOffsets[4] = -(bound.top + bound.bottom) / 2;
        paint2.getTextBounds(text6,0,text6.toString().length(),bound);
        yOffsets[5] = -(bound.top + bound.bottom) / 2;
